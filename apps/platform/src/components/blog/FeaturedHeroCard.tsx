...
export function FeaturedHeroCard({
  ...
  return (
    <Link href={`/blog/${slug}`} className="block group FeaturedHeroCard">
      <article className="flex flex-col md:flex-row gap-6 sm:gap-8 py-6 px-0 transition-all duration-200 border-b border-border last:border-0 hover:bg-neutral-50/50 dark:hover:bg-neutral-950/50">
        {/* Left Column: Cover Image (~60% width on desktop) */}
        <div className="w-full md:w-3/5 flex-shrink-0">
          <div className="relative aspect-[16/9] overflow-hidden bg-neutral-100 dark:bg-neutral-800 rounded-sm">
            <Image
              src={imageSrc}
              alt={title}
              fill
            ...
            {premium && (
              <div className="absolute top-3 left-3">
                <Badge
                  variant="secondary"
                  className="text-sm px-3 py-1.5 bg-primary text-primary-foreground font-ui"
                >
        ...
        {/* Right Column: Content (~40% width on desktop) */}
        <div className="w-full md:w-2/5 flex flex-col justify-between">
          <div className="space-y-3">
            {/* Category and Content Type */}
            <div className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400 font-ui">
              <span className="px-3 py-1.5 text-sm font-medium border border-border text-neutral-800 dark:text-neutral-200 font-ui rounded-none">
                {category}
              </span>
              {content_type === "research_paper" && (
                <Badge
                  variant="outline"
                  className="text-sm px-2 py-1 border-primary/20 text-primary bg-primary/5 font-ui"
                >
                  Research
                </Badge>
              )}
            ...
            {/* Title */}
            <h3 className="text-2xl sm:text-3xl font-headings font-bold text-neutral-900 dark:text-white line-clamp-3 group-hover:text-neutral-700 dark:group-hover:text-neutral-300 transition-colors duration-200">
              {title}
            </h3>

            {/* Excerpt */}
            <p className="text-neutral-600 dark:text-neutral-400 text-base sm:text-lg line-clamp-4 font-body leading-relaxed">
              {excerpt}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50">
            ...
            <div className="flex items-center">
              <span className="text-sm text-neutral-500 dark:text-neutral-500 font-ui">
                {formattedDate}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 text-sm text-neutral-600 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors duration-200 p-1.5 rounded-none border border-transparent hover:border-border"
                title="Share post"
              >
