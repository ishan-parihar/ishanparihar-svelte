...
export function HorizontalBlogCard({
  ...
  return (
    <Link href={`/blog/${slug}`} className="block group HorizontalBlogCard">
      <article className="flex flex-row gap-4 sm:gap-6 py-4 px-0 transition-all duration-200 border-b border-border last:border-0 hover:bg-neutral-50/50 dark:hover:bg-neutral-950/50">
        {/* Left Column: Cover Image (~40% width) */}
        <div className="w-2/5 flex-shrink-0">
          <div className="relative aspect-[4/3] overflow-hidden bg-neutral-100 dark:bg-neutral-800 rounded-sm">
            <Image
              src={imageSrc}
              alt={title}
              fill
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-neutral-600 dark:text-neutral-400 font-ui">
              <span className="px-3 py-1.5 text-xs font-medium border border-border text-neutral-800 dark:text-neutral-200 font-ui rounded-none">
                {category}
              </span>
              {content_type === "research_paper" && (
                <Badge
                  variant="outline"
                  className="text-xs px-2 py-1 border-primary/20 text-primary bg-primary/5 font-ui"
                >
                  Research
                </Badge>
              )}
            ...
            {/* Title */}
            <h3 className="text-xl sm:text-2xl font-headings font-bold text-neutral-900 dark:text-white line-clamp-2 group-hover:text-neutral-700 dark:group-hover:text-neutral-300 transition-colors duration-200">
              {title}
            </h3>

            {/* Excerpt */}
            <p className="text-neutral-600 dark:text-neutral-400 text-sm sm:text-base line-clamp-3 font-body leading-relaxed">
              {excerpt}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mt-2 pt-1 border-t border-border/50">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs text-neutral-600 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors duration-200 p-1.5 rounded-none border border-transparent hover:border-border"
                title="Share post"
              >
