...
<div class={`pl-${depth * 5}`}>
  <div class="relative">
    {#if depth > 0}
      <div class="absolute top-0 bottom-0 w-[2px] border-gray-300 dark:border-gray-600 cursor-pointer opacity-60 hover:opacity-100 transition-opacity" style="left: 8px;" on:click={toggleCollapse}></div>
        ...
        <div>
          ...
          {#if !comment.is_deleted}
            <div class="flex items-center gap-1.5 mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {#if $user}
                <Button on:click={handleReplyClick} variant="ghost" size="sm" class="h-6 px-2 text-xs font-medium">Reply</Button>
              {:else}
                <Button variant="ghost" size="sm" class="h-6 px-2 text-xs font-medium" on:click={() => { /* TODO: open auth modal */ }}>Sign in to reply</Button>
              {/if}
              {#if comment.replies && comment.replies.length > 0}
                <Button on:click={toggleCollapse} variant="ghost" size="sm" class="h-6 px-2 text-xs font-medium">
            ...
            <div class="mt-1.5">
              <div class="space-y-2">
                ...
                <div class="flex justify-end gap-2">
                  <Button size="sm" variant="secondary" on:click={() => isEditing = false} class="h-8 text-xs font-medium"><X class="h-3.5 w-3.5 mr-1" /> Cancel</Button>
                  <Button size="sm" variant="default" on:click={handleEditComment} class="h-8 text-xs font-medium"><Check class="h-3.5 w-3.5 mr-1" /> Save</Button>
                </div>
              </div>
            </div>
          {/if}
          {#if comment.replies && comment.replies.length > 0 && !isCollapsed}
            <div class="mt-1.5">
              {#each comment.replies as reply}
                <svelte:self comment={reply} {blogPostId} {onCommentAdded} depth={depth + 1} {maxDepth} />
              {/each}
            </div>
          {/if}
        </div>
