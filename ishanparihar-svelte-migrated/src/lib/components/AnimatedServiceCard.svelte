<script lang="ts">
  ...
  const formatPrice = () => {
    ...
    if (pricingType === "recurring" && billingPeriod) {
      ...
      return `${formattedPrice}${periodMap[billingPeriod as string]}`;
    }
  ...
  const getServiceTypeInfo = () => {
    ...
    return typeMap[serviceType as string];
  };
  ...
  <LinkLoadingWrapper href={`/offerings/${slug}`} showIndicator={true} class="block">
    <div class="group overflow-hidden hover:shadow-lg transition-all duration-300 bg-card border border-border rounded-none">
         <div class="relative aspect-[4/3] overflow-hidden">
          <CoverImage src={coverImage} alt={title} className="transition-transform duration-300 group-hover:scale-105" {priority} width={undefined} height={undefined} />
